...
var FancyUpload2 = new Class({
	...
	onLoad: function() {
		this.log('Uploader prêt!');
	},

	onBeforeOpen: function(file, options) {
		this.log('Initialisation upload pour "{name}".', file);
	...
	onOpen: function(file, overall) {
		...
		this.currentTitle.set('html', 'Progression du fichier "{name}"'.substitute(file) );
	},

	onProgress: function(file, current, overall) {
		this.overallProgress.start(overall.bytesLoaded, overall.bytesTotal);
		this.currentText.set('html', 'Uploader à {rate}/s. Temps: ~{timeLeft}'.substitute({
	...
	onComplete: function(file, response) {
		...
		this.currentText.set('html', 'Upload terminée!');
	...
	updateOverall: function(bytesTotal) {
		...
		this.overallTitle.set('html', 'Progression globale (' + this.sizeToKB(bytesTotal) + ')');
	},
