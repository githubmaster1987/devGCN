...
if($_GET['action'] == "del")
{
	 unlink('docspatients/' . $_GET['liendoc']);
	 $sql = "DELETE FROM `DocPatients` WHERE `DocPatients`.`IdDoc` = ".$_GET['IdDoc']." LIMIT 1";
	 mysql_query($sql);
	 header("Location:ManageDocs.php?PatientId=".$_GET['PatientId']);
...
if(!isset($_GET['PatientId']))
	...
	$sqldoc = "SELECT p.Nom 'nompatient', p.Prenom, pp.IdDoc, pp.Nom 'DocName', pp.Lien"
        . " FROM `DocPatients` pp, `Patients` p"
        . " Where pp.IdPatient = p.Id"
        . " And pp.`IdPatient` = " . $id; 
	$resdoc = mysql_query($sqldoc);

	while($datadoc = mysql_fetch_array($resdoc)){
			$nom = $datadoc['Nom'];
			$prenom = $datadoc['Prenom'];
			$lien[] = "<a style=\"color:#5782af;\" href='docspatients/".$datadoc['Lien']."' target='blank'>".$datadoc['DocName']."</a>&nbsp;&nbsp;"; 
			$DelLien[] = "&nbsp;&nbsp;<a onclick=confirmDel('ManageDocs.php?PatientId=".$id."&action=del&IdDoc=".$datadoc['IdDoc']."&liendoc=".$datadoc['Lien']."') href='#'><img src=\"images/cancel.png\" title=\"Supprimer\" border=\"0\" width=\"16\" height=\"16\"></a>";
		} 
	include("header.html");
?>
	<style type="text/css">
...
window.addEvent('load', function() {
	...
	<div id="content" align="center" style="padding-bottom:20px;background-image:url('images/bg_repeat.png'); background-repeat:repeat-y; width:806;">
			...
			<center><h3>Gestion documents de <?php echo strtoupper($sqldata['Nom'])." ". ucfirst(strtolower($sqldata['Prenom'])) ?></h3><br /><br /></center>
		...
		<div id="demo">
			<form action="Uploader/scriptDoc.php?PatientId=<?php echo $id; ?>" method="post" enctype="multipart/form-data" id="form-demo">
				...
				<tr>
					<td valign="top">
						<table border="0" cellspacing="0" cellpadding="0">
									...
									{
									echo "<tr><td>&bull;&nbsp;" .$lien[$i].' </td><td> '.$DelLien[$i] ."</td></tr>"; // on affiche l'élément du tableau d'indice $i
									}
									?>
